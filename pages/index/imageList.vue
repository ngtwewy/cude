  ...
  <view class="news-container">
    ...
    <!-- <view class="tab-bar-container"> -->
      <scroll-view
        ...
        >
        <view v-for="(tab,index) in tabBars" :key="index" class="tab-item" :id="tab.id" :data-current="index" @click="ontabtap">
  ...
  export default {
    ...
    methods: {
      ontabtap(e) {
        ...
        this.tagId = this.scrollInto.slice(3);
        this.articleList = [];
      ...
      getArticleCategory(){
        var that = this;
        this.isLoading = true;
        this.$http.get(config.api.url + '/wp/v2/categories?parent=6')
          .then(res=>{
            if(res.statusCode == 200){
              that.tabBars = res.data.map(function(item,index){
                return {id:'id-'+item.id.toString(), name:item.name};
              });
              that.tabBars.unshift({name:"全部",id:'id-0'});
            }
...
<style lang="scss">
  ...
  .list-container{width: 690rpx; margin-left:30rpx;margin-top: 80rpx;}
